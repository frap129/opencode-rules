...
5. **Earliest Dispatch**: A newly Matched Durable Rule in the file-observation family (`globs`, `fileContains`, or both) is admitted immediately through an awaited `session.synthetic` message (`resume: false`, no description, so it never triggers a reply or surfaces as a user turn or UI notice); pending guidance is included transiently in the next usable dispatch and persistence retries there. Initial Durable matches are published through `session.synthetic` on the session `prompt` hook with `resume: false` (the hook fires before the user message is dispatched, so a scheduled run would see the rule block alone), making synthetic admission the single durable channel.
6. **Rule Delivery**: Each injection event is one `<system-message>` block with one plugin preamble and a `<rule name="...">` block per rule. The name comes from frontmatter `name` or the filename stem. Session-durable rules (unconditional, `globs`, `fileContains`, `keywords`, `command`, `project`, `os`, and `ci`) are published once per turn as one persisted synthetic message via `session.synthetic` on the session `prompt` hook (`resume: false`, no description, hidden in the TUI but included in the next provider request). Agent, `model`, `branch`, and `tools` rules are appended only to the model request as one transient synthetic message per matching turn via the session `context` hook, so changing agent or model does not leave stale rule text in new history. Path-based deduplication applies only to durable delivery.
...
3. **session `prompt` hook** - User prompt capture and durable synthetic rule delivery
   ...
   - Publishes all newly matched **session-durable** rules and durable hook guidance as one hidden synthetic message via `session.synthetic` (`resume: false`, no description) for the dispatch, with an id derived from the codec part identity; ephemeral matches (agent, model, branch, tools) are never written here
   - `resume: false` is required because the prompt hook runs before the user message is dispatched; a scheduled run would execute on a context containing only the rule block
   - Synthetic messages are hidden in the TUI but included in provider requests, keeping the system prompt byte-stable across requests for provider prompt caching
