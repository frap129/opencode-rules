...
- Use Aube and install with `aube install --frozen-lockfile`. Run checks in this order: `aube run lint` -> `aubex tsc --noEmit` -> `aube run test:run`.
- There is no `typecheck` script; typecheck with `aubex tsc --noEmit`.
